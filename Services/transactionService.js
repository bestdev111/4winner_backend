...
const getAllTransactions = async (user) => {
  try{
    if(user.userRole.priority == 4)
      return await getTransaction(user)
    else if(user.userRole.priority < 4){
      let transactions = await getTransaction(user)
      let users = await User.find({parent: user._id})
      for(let idx = 0; idx < users.length; idx ++)
        transactions = await transactions.concat(await getAllTransactions(users[idx]))
      return transactions
    }
  }catch(err){
    throw err;
...
const getTransaction = async (user) => {
  try{
    ...
    if(user.userRole.priority == 1)
      ...
      transactions = await Transaction.find({ distributor: user._id, type: 34})
      .populate('admin')
      .populate('agent')
      .populate('distributor')
      .populate('cashier')
      ...
      transactions = await Transaction.find({ cashier: user._id, type: 45})
      .populate('admin')
      .populate('agent')
      .populate('distributor')
      .populate('cashier')
